...
def main():
    ...
    if args.compute_lpips:
        lpips_values = lpips_per_image(
            pred=pred, target=target, batch_size=args.batch_size, device=device, net=args.lpips_net
        )
    ...
    if lpips_values is not None:
        print(f"LPIPS mean={summary['metrics']['lpips']['mean']:.4f}  std={summary['metrics']['lpips']['std']:.4f}")

    rows = []
    for idx, name in enumerate(names):
        row = {"index": idx, "name": name, "psnr": float(psnr_values[idx]), "ssim": float(ssim_values[idx])}
        if lpips_values is not None:
