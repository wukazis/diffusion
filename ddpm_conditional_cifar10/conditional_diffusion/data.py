    ...
    *,
    ...
    if dataset_name == "cifar10":
        ...
        if class_cond:
            ...
            sorted_classes = {
                name: idx for idx, name in enumerate(sorted(set(class_names)))
            }
...
class CIFAR10Dataset(Dataset):
    def __init__(
        ...
        self.indices = list(range(len(self.dataset)))[shard:][::num_shards]

    def __len__(self):
        return len(self.indices)

    def __getitem__(self, idx):
        pil_image, label = self.dataset[self.indices[idx]]
