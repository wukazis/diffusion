...
class FMUNetModel(nn.Module):
    def __init__(
        ...
        self.label_emb = (
            nn.Embedding(num_classes + 1, time_embed_dim)
    ...
    def forward(self, x, t, y=None):
        ...
        if self.label_emb is None:
            if y is not None:
                raise ValueError("received class labels for an unconditional model")
        else:
            if y is None:
                raise ValueError("class labels are required for conditional generation")
            if y.ndim == 0:
                y = y[None]
            ...
            if self.training and self.class_dropout_prob > 0:
                drop_mask = th.rand_like(y.float()) < self.class_dropout_prob
                null_mask = null_mask | drop_mask
            if self.num_classes is not None:
                invalid_mask = (y >= self.num_classes) & ~null_mask
                if invalid_mask.any():
                    raise ValueError("class labels exceed configured num_classes")
            y = y.clamp(min=0)
            if self.null_class is not None:
                y = th.where(null_mask, th.full_like(y, self.null_class), y)
