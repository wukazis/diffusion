...
from flow_matching.data import create_data_loader
...
def main():
    ...
    logger.log("creating paired FM model...")
    ...
    if args.save_png_samples > 0:
        ...
        logger.log(
            f"saving {min(args.save_png_samples, len(pred_arr))} generated PNGs to {pred_dir}"
        )
        save_png_samples(pred_arr, pred_dir, args.save_png_samples)

        triplet_dir = os.path.join(logger.get_dir(), "triplet_panels")
        logger.log(
            f"saving {min(args.save_png_samples, len(pred_arr))} triplet panels to {triplet_dir}"
        )
