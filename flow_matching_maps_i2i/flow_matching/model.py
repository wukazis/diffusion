...
class FMUNetModel(nn.Module):
    def __init__(
        ...
        self.in_conv = nn.Conv2d(
            in_channels + cond_channels, model_channels, 3, padding=1
        )
        ...
        self.middle_blocks = nn.ModuleList(
            [
                FMResBlock(ch, ch, time_embed_dim, dropout),
                FMResBlock(ch, ch, time_embed_dim, dropout),
            ]
        )
    ...
    def forward(self, x, t, cond=None):
        if cond is None:
            raise ValueError("cond image is required for image-to-image generation")
        if x.shape[0] != cond.shape[0]:
            raise ValueError("cond batch size must match x batch size")
        if cond.shape[1] != self.cond_channels:
            raise ValueError(
                f"expected cond to have {self.cond_channels} channels, got {cond.shape[1]}"
            )
        if cond.shape[2:] != x.shape[2:]:
            raise ValueError("cond spatial size must match x")
        if t.ndim == 0:
            t = t[None]

        emb = self.time_embed(timestep_embedding(t * 1000.0, self.model_channels))
        h = self.in_conv(th.cat([x, cond], dim=1))
        skips = []
